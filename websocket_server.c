...
static void background_callback(struct netconn *conn, enum netconn_evt evt,
                                u16_t len) {
  switch (evt) {
...
int ws_server_add_client_protocol(struct netconn *conn, char *msg, uint16_t len,
...
int ws_server_add_client(struct netconn *conn, char *msg, uint16_t len,
                         char *url,
...
int ws_server_send_text_clients_from_callback(char *url, char *msg,
  ...
  for (int i = 0; i < WEBSOCKET_SERVER_MAX_CLIENTS; i++) {
    if (clients[i].url != NULL && ws_is_connected(clients[i]) &&
        !strcmp(clients[i].url, url)) {
